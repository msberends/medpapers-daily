...
def _save_config(path: Path, new_data: dict):
    """Save config.yaml, preserving comments if ruamel.yaml is available."""
    if _HAS_RUAMEL:
        existing_cm = None
        if path.exists():
            with open(path) as f:
...
async def admin_page(
    ...
    with conn_ctx() as conn:
        ...
        papers_total_pages = max(1, (papers_total + _ADMIN_PAPERS_PAGE_SIZE - 1) // _ADMIN_PAPERS_PAGE_SIZE)
        papers_page = max(1, min(papers_page, papers_total_pages))
        papers_offset = (papers_page - 1) * _ADMIN_PAPERS_PAGE_SIZE
        papers = conn.execute(
            """SELECT p.pmid, p.title, p.journal, p.publisher, p.pub_date, p.epub_date,
               ...
               LIMIT ? OFFSET ?""",
            (_ADMIN_PAPERS_PAGE_SIZE, papers_offset),
        ).fetchall()
        all_publishers = [
    ...
    return request.app.state.templates.TemplateResponse(request, "admin.html", {
        ...
        "papers_total_pages": papers_total_pages,
        "all_publishers": all_publishers,
        "elsevier_api_key": config.get("elsevier_api_key", ""),
        "valid_themes": VALID_THEMES,
        "valid_timezones": __import__("zoneinfo").available_timezones(),
        "config": config,
...
async def save_config(request: Request):
    ...
    # Start from existing so unknown keys (llm_*, custom) are preserved unchanged.
    new_config = {
        **existing,
    ...
    _save_config(BASE_DIR / "config.yaml", new_config)

    request.app.state.config = new_config
    return flash_redirect("/admin", "Configuration saved.")
...
async def test_email(request: Request):
    ...
    to = cfg.get("email", "")
    if not to:
        return flash_redirect("/admin#tab-email", "No email address configured for your account.", "danger")
    ...
    try:
        ...
        return flash_redirect("/admin#tab-email", str(e), "danger")
    return flash_redirect("/admin#tab-email", f"Test email sent to {to}.")
...
async def create_user(request: Request):
    ...
    if not username or not password:
        return flash_redirect("/admin#tab-users", "Username and password required.", "danger")
    now = datetime.now(timezone.utc).isoformat()
    with conn_ctx() as conn:
        if conn.execute("SELECT id FROM users WHERE username = ?", (username,)).fetchone():
            return flash_redirect("/admin#tab-users", f"User {username} already exists.", "danger")
        conn.execute(
    ...
    return flash_redirect("/admin#tab-users", f"User {username} created.")
...
async def delete_user(user_id: int, request: Request):
    ...
    if current and current["user_id"] == user_id:
        return flash_redirect("/admin#tab-users", "Cannot delete yourself.", "danger")
    with conn_ctx() as conn:
        conn.execute("DELETE FROM users WHERE id = ?", (user_id,))
    return flash_redirect("/admin#tab-users", "User deleted.")
...
async def run_fetch(request: Request):
    ...
    try:
        ...
        return flash_redirect("/admin#tab-fetch", "Fetch started in the background. Check Logs for progress.", "info")
    except Exception as e:
        return flash_redirect("/admin#tab-fetch", str(e), "danger")
...
async def run_digest(request: Request):
    ...
    try:
        ...
        return flash_redirect("/admin#tab-email", "Email digest started in the background. Check Logs for results.", "info")
    except Exception as e:
        return flash_redirect("/admin#tab-email", str(e), "danger")
...
async def delete_papers(request: Request):
    ...
    if not pmid_list:
        return flash_redirect("/admin#tab-papers", "No papers selected.", "warning")
    ...
    return flash_redirect("/admin#tab-papers", f"{len(pmid_list)} paper(s) permanently deleted.")
...
async def save_publisher_map(request: Request):
    ...
    _save_config(BASE_DIR / "config.yaml", config)
    return flash_redirect("/admin#tab-publishers", "Publisher map saved.")
...
async def save_llm_config(request: Request):
    ...
    _save_config(BASE_DIR / "config.yaml", config)
    request.app.state.config = config
    return flash_redirect("/admin#tab-llm", "LLM configuration saved.")
...
async def run_llm_highlights(request: Request):
    ...
    try:
        ...
        return flash_redirect("/admin#tab-llm", "LLM highlights generation started in the background. Check Logs for progress.", "info")
    except Exception as e:
...
async def reset_llm_status(request: Request):
    ...
    try:
        ...
        pass
    return flash_redirect("/admin#tab-llm", "LLM status reset.")
