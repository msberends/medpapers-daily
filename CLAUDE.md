...
## Multi-tab form pattern

Every multi-tab settings page (user Settings, admin user editor) uses **one POST endpoint per tab**. Each endpoint:
1. Loads the current YAML with `existing = _load_user_cfg(username)`.
2. Builds the new config as `{**existing, ...only_fields_owned_by_this_tab}`.
3. Saves and redirects back with `?tab=<this-tab>` so the user lands on the correct tab.

Because `**existing` preserves every key not explicitly overwritten, **no `<input type="hidden">` passthroughs are needed or wanted**. Do not add them.

The only hidden inputs that are acceptable are **row identifiers** (e.g. `profile_id` in the Search Profiles form, which is required to match existing DB rows on update) — not copies of values owned by another tab.

**Rule: when you add a new user-facing setting:**
1. Add it as a real input in the tab that owns it.
2. Add it to that tab's endpoint only — `**existing` handles preservation for all other tabs automatically.
3. Do not add passthrough hidden inputs anywhere.
